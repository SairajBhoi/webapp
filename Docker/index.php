  ...
  /* If input fields are populated, add a row to the EMPLOYEES table. */
{
  ...
  if ( strlen($employee_id) || strlen($employee_name) || strlen($employee_address)) {
    AddEmployee($connection,$employee_id, $employee_name, $employee_address);
  }
}
?>
...
function AddEmployee($connection,$id ,$name, $address) {
   $i=  mysqli_real_escape_string($connection, $id);
   $n = mysqli_real_escape_string($connection, $name);
   $a = mysqli_real_escape_string($connection, $address);

   $query = "INSERT INTO EMPLOYEES (ID,NAME, ADDRESS) VALUES ('$i','$n', '$a');";
